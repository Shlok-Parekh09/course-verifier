...
class DatabaseManager:
    ...
    def initialize_db(cls):
        ...
        # University Abbreviations table
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS university_abbreviations (
    ...
    @classmethod
    def get_all_abbreviations(cls) -> dict:
        """Get all abbreviations"""
        conn = cls.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('SELECT abbreviation, full_name FROM university_abbreviations')
            return {row['abbreviation']: row['full_name'] for row in cursor.fetchall()}
        except Exception:
            return {}

    @classmethod
    def insert_abbreviation(cls, abbr: str, full_name: str):
        conn = cls.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('''INSERT OR REPLACE INTO university_abbreviations (abbreviation, full_name) VALUES (?, ?)''', (abbr, full_name))
            conn.commit()
        except Exception as e:
            print(e)

    @classmethod
    def get_affiliation(cls, course_name: str, university: str) -> str:
        conn = cls.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('SELECT affiliated_uni FROM affiliations WHERE course_name = ? AND university = ?', (course_name, university))
            row = cursor.fetchone()
            return row['affiliated_uni'] if row else None
        except Exception:
            return None

    @classmethod
    def insert_affiliation(cls, course_name: str, university: str, affiliated_uni: str):
        conn = cls.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('''INSERT OR REPLACE INTO affiliations (course_name, university, affiliated_uni) VALUES (?, ?, ?)''', (course_name, university, affiliated_uni))
            conn.commit()
        except Exception as e:
            print(e)
