...
for idx, chunk in enumerate(chunks):
    ...
    yaml_lines.extend([
        ...
        "  image: python:3.12",
        "  tags:",
        f"    - {os.environ.get('RUNNER_TYPE', 'saas-linux-small-amd64')}",
        "  variables:",
        f"    START_PAGE: \"{start}\"",
        f"    END_PAGE: \"{end}\"",
        "    CI: \"true\"",
        ...
        "      chunk_pdf=\"link_compile_AUTONOMOUS_VERIFIED.pdf\"",
        "      if [ -f \"$chunk_pdf\" ]; then",
        "          curl -sT \"$chunk_pdf\" \"https://transfer.sh/${START_PAGE}_to_${END_PAGE}_$chunk_pdf\" > \"url_${START_PAGE}_to_${END_PAGE}.txt\"",
        "      fi",
        "      mv autonomous_verified_link_compile.pdf.json \"autonomous_verified_${START_PAGE}_to_${END_PAGE}_link_compile.json\" || true",
        "      mv local_database.db \"local_database_${START_PAGE}_to_${END_PAGE}.db\" || true",
...
yaml_lines.extend([
    ...
    "    - |",
    "      cd backend",
    "      for url_file in url_*.txt; do",
    "          if [ -f \"$url_file\" ]; then",
    "              url=$(cat \"$url_file\")",
    "              chunk_name=\"verification-results-${url_file#url_}\"",
    "              chunk_name=\"${chunk_name%.txt}-dummy.pdf\"",
    "              echo \"Downloading $url to $chunk_name\"",
    "              curl -sL \"$url\" -o \"$chunk_name\" || true",
    "          fi",
    "      done",
    "      cd ..",
    "    - |",
    ...
    "      DBEOF",
    "    - |",
    "      if [ -f merged_run.pdf ]; then",
    "          echo '=================================================='",
    "          echo 'Uploading final merged PDF to transfer.sh...'",
    "          FINAL_URL=$(curl -sT merged_run.pdf https://transfer.sh/merged_run.pdf || echo 'Upload failed')",
    "          echo '📄 DOWNLOAD YOUR MERGED REPORT PDF HERE:'",
    "          echo \"$FINAL_URL\"",
    "          echo '=================================================='",
    "      fi",
    "  artifacts:",
