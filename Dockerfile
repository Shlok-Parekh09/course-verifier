...
# Expose the default port; Render injects $PORT at runtime
EXPOSE 8080

# Default: run the dashboard via gunicorn so it works on Render/Docker/Okteto
CMD exec gunicorn -b 0.0.0.0:$PORT --workers 4 --threads 2 --timeout 120 wsgi:app
